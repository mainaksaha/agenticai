...
async def run_queries(fin_server, util_server, model_server, party_server):
    ...
    # Handoff manager chains sub-agents
    advisor_agent = Agent(
        name="FinancialAdvisorAgent",
        handoffs=[fin_agent, util_agent, model_agent, party_agent, research_agent],
        #instructions="You are a financial advisor assistant that delegates tasks to specialized agents.",
    )
    ...
    # Run each query through the handoff agent
    for query in queries:
        print(f"\n=== Query: {query}")
        result = await Runner.run(starting_agent=advisor_agent, input=query)
...
async def main():
    ...
    async with MCPServerSse(name="FinancialServer", params={"url": FINANCIAL_MCP_SSE_URL}) as fin, \
        ...
        with trace(workflow_name="Handoff-MCP CLI Session", trace_id=trace_id):
